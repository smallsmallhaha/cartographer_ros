...
void Run(const string &pose_graph_filename,
  ...
  for (const rosbag::MessageInstance& message : view) {
      ...
      if(message.getTopic().compare("/horizontal_laser_2d")==0)
      {
          ...
          auto laser_scan_message=*message.instantiate<sensor_msgs::LaserScan>();
          
          const carto::common::Time start_time = FromRos(laser_scan_message.header.stamp);
  ...
  const carto::transform::TransformInterpolationBuffer transform_interpolation_buffer(pose_graph_proto.trajectory(0));
...
//     cout << "submap_id:  " << submap_id.trajectory_id() << "," << submap_id.submap_index() << endl;
//     cout << "node_id:  " << node_id.trajectory_id() << "," << node_id.node_index() << endl;
//     cout << "relative_pose:" << "" << endl;
